...
var map = L.map('map-canvas'),
    realtime = L.realtime({
        url: baseurl +'realtime_unit_position/getListUnit',
        crossOrigin: true,
        type: 'json'
    }, {
        interval: 3 * 1000, 
...
realtime.on('update', function() {
    ...
    function getalldeviceonlinetoday() {
        ...
        $.ajax({
            url: baseurl +'realtime_unit_position/getalldeviceonlinetoday',
            type: 'GET',
            dataType: 'json',
    ...
    function getAllRoutesForMap() {
       ...
       // routeSelect.selectedIndex = 0;
        ...
        $.ajax({
            url: baseurl +'realtime_unit_position/getallroutesformap',
            type: 'GET',
            dataType: 'json',
            success: function(data) {
                loadGPSLocations(data);
    ...
    function loadRoutesIntoDropdownBox() {      
        $.ajax({ 
            url: baseurl +'realtime_unit_position/getroutes',
            type: 'GET',
            dataType: 'json',
            success: function(data) {
            loadRoutes(data);
    ...
    function getRouteForMap() { 
        if (hasMap()) {
            ...
            $.ajax({
                   url: baseurl +'realtime_unit_position/getrouteformap/' + $('#routeSelect').val(),
                   type: 'GET',
                   dataType: 'json',
                   success: function(data) {
                      loadGPSLocations(data);
    ...
    function loadGPSLocations(json) {
        ...
        if (json.length == 0) {
            ...
            if (map.id == 'map-canvas') {
                ...
                var gpsrealtime_unit_positionMap = new L.map('map-canvas');
               ...
               //  var bingMapsLayer = new L.BingLayer("AnH1IKGCBwAiBWfYAHMtIfIhMVybHFx2GxsReNP5W0z6P8kRa67_QwhM4PglI9yL");
                ...
                L.polyline([[0, 0], ]).addTo(gpsrealtime_unit_positionMap);

                // this sets which map layer will first be displayed
                gpsrealtime_unit_positionMap.addLayer(googleMapsLayer);

                // this is the switcher control to switch between map types

                //'Bing Maps':bingMapsLayer,
                gpsrealtime_unit_positionMap.addControl(new L.Control.Layers({
                ...
                $(json.locations).each(function(key, value){
                    ...
                    if (counter == $(json.locations).length) {
                        //gpsrealtime_unit_positionMap.setView(tempLocation, zoom);  if using fixed zoom
                        finalLocation = true;
                    ...
                    var marker = createMarker(
                        ...
                        $(this).attr('accuracy'),
                        $(this).attr('extraInfo'),
                        gpsrealtime_unit_positionMap, finalLocation);
                });
                
                // fit markers within window
                var bounds = new L.LatLngBounds(locationArray);
                gpsrealtime_unit_positionMap.fitBounds(bounds);
    ...
    function createMarker(latitude, longitude, speed, direction, distance, locationMethod, gpsTime,
        ...
        var popupWindowText = "<table border=0 style=\"font-size:95%;font-family:arial,helvetica,sans-serif;color:#000;\">" +
            "<tr><td align=right>&nbsp;</td><td>&nbsp;</td><td rowspan=2 align=right>" +
            "<img src="+baseurl+"gpsrealtime_unit_position/assets/images/" + getCompassImage(direction) + ".jpg alt= />" + lastMarker +
        ...
        var gpsrealtime_unit_positionMarker;
        var title = userName + " - " + gpsTime;

        // make sure the final red marker always displays on top 
        if (finalLocation) {
            gpsrealtime_unit_positionMarker = new L.marker(new L.LatLng(latitude, longitude), {title: title, icon: markerIcon, zIndexOffset: 999}).bindPopup(popupWindowText).addTo(map);
        } else {
            gpsrealtime_unit_positionMarker = new L.marker(new L.LatLng(latitude, longitude), {title: title, icon: markerIcon}).bindPopup(popupWindowText).addTo(map);
        }
        
        // if we are viewing all routes, we want to go to a route when a user taps on a marker instead of displaying popupWindow
        if (viewingAllRoutes) {
            gpsrealtime_unit_positionMarker.unbindPopup();
            
            gpsrealtime_unit_positionMarker.on("click", function() {         
                var url = baseurl +'realtime_unit_position/getrouteformap/' + sessionID;

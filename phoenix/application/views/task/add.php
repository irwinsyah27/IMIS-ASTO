	...
	<form class="form-horizontal" method="POST" enctype="multipart/form-data">
		<div class="panel-body">
			<div class="form-group <?= form_error('task[title]') ? 'has-error' : '' ?>">
				...
				<div class="col-md-5">
					<input type="text" name="task[title]" value="<?= set_value('task[title]') ?>" class="form-control" placeholder="Task Title">
			...
			<div class="form-group <?= form_error('task[description]') ? 'has-error' : '' ?>">
				...
				<div class="col-md-5">
					<textarea name="task[description]" rows="4" class="form-control" placeholder="Description"><?= set_value('task[description]') ?></textarea>
			...
			<div class="form-group <?= form_error('task[priority]') ? 'has-error' : '' ?>">
				...
				<div class="col-md-5">
					...
					<select class="form-control" name="task[priority]">
						...
						<option value="<?= $i ?>" <?= set_select('task[priority]', $i) ?>><?= $v ?></option>
						<?php endforeach ?>
					</select>
			...
			<div class="form-group <?= form_error('task[owner_id]') ? 'has-error' : '' ?>">
				...
				<div class="col-md-5">
					...
					<select class="chosen-select form-control" name="task[owner_id]" data-placeholder="Pilih PIC">
						...
						<option value="<?= $p->user_id ?>" <?= set_select('task[owner_id]', $p->user_id) ?>>
							<?= $p->nama ?>
						</option>
						<?php endforeach ?>
					</select>

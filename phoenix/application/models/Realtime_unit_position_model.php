<?php
class Realtime_unit_position_model extends MY_MODEL { 
	...
	public function getLokasi() {
		$sql = "
				...
				WHERE
					cast(a.time_stamp as date) = '".date("Y-m-d")."'
				";
				//echo $sql; limit 1
	...
	public function get_tracker_uniq() {
		...
		$sql = "
				...
				FROM 
					current_unit_position a 
				WHERE  
					cast(a.time_stamp as date) = '".$date."' 
 ...
 * 
 SELECT  
				...
				WHERE  
					 b.date = '2016-10-25' 
				GROUP BY a.unit, a.latitude, a.longitude,a.speed,a.heading, a.time_stamp, a.nip, a.unit_position_id
				
		$sql = "
				SELECT 
					
					DISTINCT(a.sessionId), 
					MAX(a.gpsTime) max_gpsTime,
					CAST(a.latitude AS CHAR) as latitude,
					CAST(a.longitude AS CHAR) as longitude, 
					CAST(a.speed AS CHAR) as speed, 
					CAST(a.direction AS CHAR) as direction, 
				...
				FROM 
					gpslocations a
				INNER JOIN 
					(SELECT sessionId, MAX(GPSLocationID) as GPSLocationID FROM gpslocations GROUP BY sessionId) as b
				ON
					a.GPSLocationID=b.GPSLocationID
				WHERE 
					a.sessionID != '0' 
					&& CHAR_LENGTH(a.sessionID) != 0 
					&& a.gpstime != '0000-00-00 00:00:00'
   ...
   VALUES ('".$params['latitude']."', '".$params['longitude']."', '".$params['speed']."', '".$params['direction']."', '".$params['distance']."', '".$params['date']."', '".$params['locationmethod']."', '".$params['username']."', '".$params['phonenumber']."', '".$params['sessionid']."', '".$params['accuracy']."', '".$params['extrainfo']."', '".$params['eventtype']."')";
		...
		*/
		...
		/*
		$tmp_sql = "SELECT count(*) as total FROM gpslocations WHERE username = '".$params['username']."' AND gpsTime like '".date("Y-m-d H:i")."%';";
		#echo $tmp_sql;exit;
		$tmp_rs = $this->db->query($tmp_sql);
		$temp = $tmp_rs->row_array();
		if ($temp["total"] == 0) {
			
		    $sql = 'CALL prcSaveGPSLocation(
		                          \''.$params['latitude'].'\', 
		                          \''.$params['longitude'].'\', 
		                          \''.$params['speed'].'\', 
		                          \''.$params['direction'].'\', 

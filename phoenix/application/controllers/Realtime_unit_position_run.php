...
class Realtime_unit_position_run extends CI_Controller {
   	...
   	public function __construct() {
		...
		$this->data['breadcrumb'] = '<li>
							...
							<li class="active">Realtime Unit Position RUN</li>';

		//$this->load->model('proto_model');
		$this->load->model("realtime_unit_position_model_run");  

		$menu["parent_menu"] 		= "dashboard";
		$menu["sub_menu"] 			= "realtime_unit_position_run"; 
		$this->data['check_menu']	= $menu;

		# akses level
		$akses 			= $this->realtime_unit_position_model_run->user_akses("realtime_unit_position_run");
	...
	{					   
		$this->load->view("realtime_unit_position_run/view" , $this->data);
	}  

	public function getListUnit()
	{     

		$stmt = $this->realtime_unit_position_model_run->get_tracker_uniq();  
		$i = 0;
	...
	{     
		$routes = $this->realtime_unit_position_model_run->getalldeviceonlinetoday(); 
	...
	{     
		$routes = $this->realtime_unit_position_model_run->getallroutesformap(); 
	...
	{     
		...
		$routes = $this->realtime_unit_position_model_run->getrouteformap($pk); 
	...
	{     
		...
		$routes = $this->realtime_unit_position_model_run->deleteroute($pk); 
	...
	{     
		$routes = $this->realtime_unit_position_model_run->getroutes(); 
	...
	{     
		...
		$timestamp = $this->realtime_unit_position_model_run->updatelocation($params);  
	...
	{					   
		$this->data['routes'] = $this->realtime_unit_position_model_run->getLokasiTesting(); 
		$this->load->view("realtime_unit_position_run/test" , $this->data);
	}  
	public function track()
	{					   
		$start_date = trim($this->uri->segment(3,0));
        //$this->data['routes'] = $this->realtime_unit_position_model->getLokasiTesting(); 
		$this->data['unit'] = $this->realtime_unit_position_model_run->getdataunit();
		$this->data['routes'] = $this->realtime_unit_position_model_run->getLokasi($start_date); 
		$this->load->view("realtime_unit_position_run/track" , $this->data); 
/*
...
*/		
	...
	public function runtracking(){
		...
		$json = $this->realtime_unit_position_model_run->runtracking($kendaraan, $start_date_time, $end_date_time);
		header('Content-Type: application/json');
		echo $json;
	}
}

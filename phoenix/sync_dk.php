...
#$sql 	= "SELECT * FROM `log_geolocation` WHERE `nip` LIKE 'KT14010' AND `time_stamp` >= '2017-03-19 00:00:00' AND `time_stamp` <= '2017-03-19 23:00:00' ORDER BY `time_stamp` ASC ";
#$sql 	= "SELECT * FROM `log_geolocation` WHERE `nip` LIKE 'KT13143' AND `time_stamp` >= '2017-03-19 00:00:00' AND `time_stamp` <= '2017-03-19 23:00:00' ORDER BY `time_stamp` ASC ";
#$sql 	= "SELECT * FROM `log_geolocation` WHERE `nip` LIKE 'KC12087' AND `time_stamp` >= '2017-03-19 17:00:00' AND `time_stamp` <= '2017-03-20 06:00:00' ORDER BY `time_stamp` ASC ";
$sql 	= "SELECT * FROM `log_geolocation` WHERE `nip` LIKE 'KT14002' AND `time_stamp` >= '2017-03-20 17:00:00' AND `time_stamp` <= '2017-03-21 06:00:00' ORDER BY `time_stamp` ASC ";
...
if (count($rs) > 0) {
	FOREACH ($rs AS $r) {
		...
		$status					= $r["status"];				// login , logout , idle, start, overspeed, normal, geofence
		$datetime_tab			= $r["time_stamp"];
		$hm						= $r["hm"]; 	
		...
		$altitude 				= $r["altitude"];
		$accuracy 				= $r["accuracy"];
		$altitude_accuracy 		= $r["altitude_accuracy"]; 
		$heading 				= $r["heading"];
		$speed 					= $r["speed"]; 
		$time_stamp_gps 		= $r["time_stamp"];
		$id 					= $r["geolocation_id"]; 
...
function login_daily_absent($db, $nip = "", $unit = "", $device_id = "", $latitude = "", $longitude = "", $altitude = "", $accuracy = "", $altitude_accuracy = "", $heading = "", $speed = "", $time_stamp_gps = "", $datetime_tab = "", $status = "", $hm = "") {
	...
	if ($status == "login") { 
		$date 		= new DateTime($time_stamp_gps); 
		...
		if($rs_check["total"] > 0) {  
			...
			$data = array(
			    ...
			    'shift' 						=> $shift, 
			    'time_in' 						=> $time_in, 
			    'time_start_position_station' 	=> $time_stamp_gps 
			); 
...
function insert_update_current_unit_position($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="") {
	...
	$rs_check 	= $db->GetRow($sql_check); 
	if(empty($rs_check["unit_position_id"]) && $rs_check["unit_position_id"]=="") { 
		...
		$data = array(
		    ...
		    'heading' 						=> $heading , 
		    'speed' 						=> $speed , 
		    'date_last_update' 				=> $time_stamp_gps 
		); 
		...
		if ($time_stamp_gps > $rs_check["date_last_update"]) {
...
function insert_update_geofences($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="") {
	...
	if (isset($rs_cov_in["coverage_in_id"]) && $rs_cov_in["coverage_in_id"] <> "") { 
		...
		FOREACH ($rs_geofence AS $r) {
			...
			if ($distance <= $radius_geofence) { 
				...
				if ($station_id == "1" || $station_id == "2" ) {
					update_station_log_cycle_time($db, $nip , $unit , $time_stamp_gps , $station_id) ;
				} 
			}else {
				if ($id_1 > 0) { 
...
function update_station_log_cycle_time($db, $nip="", $unit="", $time_stamp_gps="", $station_id="") {
	...
	if ($rs_ct["cycle_time_id"] <> "") {
		if ($rs_ct[$field] == NULL) { 
			$sql_update_ct 	= "UPDATE log_cycle_time SET ".$field." = '".$time_stamp_gps."' WHERE cycle_time_id = '".$rs_ct["cycle_time_id"]."'";
...
function update_end_log_cycle_time($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="") {
	...
	$rs_ct	= $db->GetRow($sql_ct);
	if (isset($rs_ct["cycle_time_id"]) && $rs_ct["cycle_time_id"] <> "") {
		if (($status != "logout")) {
...
function insert_update_idle_start($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="") {
	...
	if ($status == "idle" || $status =="start") { 
		...
		if ($rs_check_jdunia["log_login_jam_dunia_id"] <> "") {
			...
			if ($status == "start") { 
				$sql_update_jdunia 	= "UPDATE log_login_jam_dunia SET date_start = '".$time_stamp_gps."' WHERE log_login_jam_dunia_id = '".$rs_check_jdunia["log_login_jam_dunia_id"]."'";
	...
	if ($status == "logout") {
		...
		if($rs_check["daily_absent_id"] <> "") { 
			...
			$data = array(
			    'date_out' 						=> $curr_date,
			    'time_out' 						=> $jam,
			    'time_stop_position_station' 	=> $time_stamp_gps ,
			    'latitude_end' 					=> $latitude, 
			    'longitude_end' 				=> $longitude, 
			    'hm_akhir' 						=> $hm
			); 
...
function checkStatusData($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="", $hm= "") {
	$SQL = "SELECT count(*) as total FROM log_geolocation WHERE device_id = '".$device_id."' AND nip = '".$nip."'  AND unit = '".$unit."'  AND time_stamp_gps = '".$time_stamp_gps."'  AND latitude = '".$latitude."' AND longitude = '".$longitude."' AND status = '".$status."' ";
	#echo $SQL."<BR>";
...
function getsaveGPSOffline($db, $con,$ord = null) { 
	$objord = json_decode($ord); 
	$whr = "";
	$nr = 0;
	$nr1 = 0;
	foreach ($objord->gpsx as $row)
	{  
		$id 					= $row->geolocation_id;
		$nip 					= $row->nip;  
		$unit 					= $row->unit;  
		$device_id 				= $row->device_id;  
		$status					= $row->status;  // login , logout , idle, start, overspeed, normal, geofence
		...
		$heading 				= $row->heading;   
		$speed 					= $row->speed;  
		$time_stamp_gps 		= $row->time_stamp_gps;  

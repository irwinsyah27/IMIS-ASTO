...
function insert_update_idle_start($db, $nip="", $unit="", $device_id="", $latitude="", $longitude="", $altitude="", $accuracy="", $altitude_accuracy="", $heading="", $speed="", $time_stamp_gps="", $datetime_tab="", $status="") {
	...
	if ($status == "idle" || $status =="start") { 
		$sql_ct = "SELECT *, count(*) as total FROM log_cycle_time WHERE nip = '".$nip."' AND unit = '".$unit."' AND datetime_start <= '". $time_stamp_gps ."'  AND datetime_end IS NULL GROUP BY cycle_time_id  LIMIT 1";
		...
		if ($rs_ct["total"] > 0) {
			...
			$sql_check_jdunia = "SELECT *, count(*) as total FROM log_login_jam_dunia WHERE nip = '".$nip."' AND unit = '".$unit."' AND DATE_FORMAT(date_stop,'%Y-%m-%d')='".$date_yesterday."' AND date_start IS NULL GROUP BY cycle_time_id  LIMIT 1";
			...
			if ($rs_check_jdunia["total"] > 0) {
				...
				$sql_check_jdunia = "SELECT *, count(*) as total FROM log_login_jam_dunia WHERE nip = '".$nip."' AND unit = '".$unit."' AND DATE_FORMAT(date_stop,'%Y-%m-%d')='".$curr_date."' AND date_start IS NULL GROUP BY cycle_time_id  LIMIT 1";

<script type="text/javascript">
			jQuery(function($) {  
				...
				$("#btnEksportDataUntukAbsensi").click(function() {
					$('#modal-eksport-data-untuk-absensi').modal({show:true});
				});
				...
				/*
				$("#submit-eksport-data-untuk-absensi").click(function() {
					//var data_post = $('#btnEksportDataUntukAbsensi').serialize();  
					//alert('A : ' + $("#terminals_id").val()); return false;

      				var data = "terminals_id=" + $("#terminals_id").val() +"&start_ua=" + $("#start_ua").val() +"&end_ua=" + $("#end_ua").val();
					$.ajax({ 
					    type: "post", 
					    data: data,
					    url: $( '#frmEksportDataUntukAbsensi' ).attr( 'action' ),
          				//url:"<?php echo site_url('changeshift_monitor/eksport_action_untuk_absensi');?>",
					    success: function( response ) {
					        console.log( response );
					        $('#modal-eksport-data-untuk-absensi').modal({show:false});
					    },	
					    error: function() {
					        alert('error handing here');
					    }
				...
				*/
				...
				.DataTable( {
					...
					"sAjaxSource": "<?php echo base_url('changeshift_monitor/get_data');?>",
					"bProcessing": true,
			        "bServerSide": true,  
					select: {
						style: 'multi'
